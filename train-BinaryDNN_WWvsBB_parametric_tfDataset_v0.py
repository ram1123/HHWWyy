...
def main():
    ...
    parser.add_argument('--scan', action='store_true', help="Run a small hyperparameter scan before training.")
    ...
    history = None
    if os.path.exists(model_path) and (not args.retrain) and (not args.scan):
        print(f"Trained model already exists at {model_path}. Loading the model...")
        model = load_model(model_path)
    else:
        if args.scan:
            # run a short scan to pick a good starting point
            model, history, best_cfg = hyperparam_scan(
                X_train, Y_train, X_val, Y_val,
                input_dim=X_train.shape[1],
                output_dir=args.output_dir,
                trials=8,              # tweak as you like
                epochs=10,
                batch_size=args.batch_size,
            )
            # Optional: fine-tune longer with more epochs
            callbacks = [
