...
def make_significance_binning_uniform(
    ...
    if total_Z <= 0.0:
        logger.debug(f"No valid binning found for fine_bins={fine_bins} with the given constraints.")
    ...
    while True:
        ...
        if can_merge:
            ...
            if need_guard_merge or (frac_drop <= tol_frac):
                ...
                logger.debug(f"Merging bins [{i}, {i+1}) and [{acc_l}, {acc_r}): frac_drop = {frac_drop:.4f}")
                acc_l = i
                S_acc, B_acc = S_m, B_m
                i -= 1
            else:
...
def scan_nbins_for_best_edges(
    ...
    for nb in nbins_list:
        ...
        logger.info(f"nbins={nb:>3}: Z_tot = {Z_tot:<2.2f}, produced bins = {produced_bins}")
...
# Plotting
# ------------------------------------
def plot_binning_scan(nb_list, Z_tot_list, S_NoWgt_bins_list, B_NoWgt_bins_list, outfile):
    try:
...
# plotting with ROOT
def plot_binning_scan_root(nb_list, Z_tot_list, S_NoWgt_bins_list, B_NoWgt_bins_list, outfile):
    import ROOT
...
if __name__ == "__main__":
    ...
    logger.info(f"Derived dnn_vbf_score_atanh range: [{score_min:.6f}, {score_upper:.6f}]")

    frac_tol = 0.005  # allow merge if local Z² drop <= 1%
    max_nbins = 15  # max number of bins to try
    nb, edges, S_NoWgt_bins, S_bins, B_NoWgt_bins, B_bins, Z_bins, Ztot = scan_nbins_for_best_edges(
        sig_score,
        bkg_score,
        sig_w,
        bkg_w,
    ...
    print("])")
    print("Per-bin (S, B, Z):")
    for i, (S_norm, S, B_norm, B, Z) in enumerate(zip(
        S_bins, S_NoWgt_bins, B_bins, B_NoWgt_bins, Z_bins
    )):
        print(
    ...
    # ------------------------------------
    with open(f"best_binning_{max_nbins}bins_{str(frac_tol).replace('.', 'p')}.txt", "w") as f:
